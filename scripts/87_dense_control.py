...
Same protocol as scripts/56 (same distracted items, same layer, same last-token edit point on the
...
If dense recovers where the sparse top-K did not (scripts/56), the negative upgrades to
"controllable at this site, but not through the SAE basis".

  CUDA_VISIBLE_DEVICES=0 python scripts/87_dense_control.py --model qwen --layer 28 --edit probe
  CUDA_VISIBLE_DEVICES=0 python scripts/87_dense_control.py --model qwen --layer 28 --edit patch
"""
...
_s56 = importlib.util.spec_from_file_location("_s56", Path(__file__).parent / "56_causal_ablation.py")
