...
is paired at the item level. Only the VT condition needs running (scripts/61 --conds VT).

    python scripts/124_build_placebo_pools.py
...
def main():
    ...
    (ROOT / "configs/placebo_v1.yaml").write_text(f"""# Placebo-caption ladder (scripts/124). Internal; VT condition only (scripts/61 --conds VT).
dm:
  split: [0.6, 0.2, 0.2]
model:
