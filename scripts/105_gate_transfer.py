...
scripts/103 cross-fits its gates *within* each pool, which cannot tell a transferable signal from a
...
As in scripts/103 only the gate is hypothetical: the patch's effect is taken from the measured runs.
Reported against the oracle gate, the always-patch policy and a matched-rate random gate.

Offline, CPU only.

    python scripts/105_gate_transfer.py
"""
