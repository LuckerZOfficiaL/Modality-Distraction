...
scripts/t8_collect.py -- so the leaderboard, grounding-law and robustness analyses extend
...
Adapters (extend REGISTRY to add models):
  ...
  (OpenAI/Anthropic: add a client class to oracle_clients.make_client, then a 'gemini'-style entry.)
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/behavioral_eval.py --model qwen2.5-vl-7b --pool dm
    python scripts/behavioral_eval.py --model gemini-3-flash --pool dm        # API, no GPU
    python scripts/behavioral_eval.py --model qwen2.5-vl-7b --pool assembled
"""
...
def make_qwen(spec, device):
    ...
    _s = importlib.util.spec_from_file_location("_c6", ROOT / "collect_dm_counterfactual_activations.py")
...
def materialize_logit_store(jsonl_path, key, pool, cfg):
    ...
    (data/activations/{key}/behavioral_{pool}/) so scripts/grounding_law_robustness.py,59 (margin)
    ...
    print(f"-> logit store {sd}  (margin: scripts/grounding_law_robustness.py,59 --model {key} --cf-subdir behavioral_{pool})")
