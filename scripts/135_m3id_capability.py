...
scripts/78 runs a single image-conditioned forward and M3ID needs two: the image-conditioned
logits l_c and the image-free logits l_u. This script caches both per row and applies the same
decoding rule scripts/128 uses, at each model's ALREADY-SELECTED (mu, alpha) -- no re-tuning, so
...
Writes data/diagnostics/capability/{benchmark}_{tag}_m3idcap_rep.json in scripts/78's format, so
...
capability delta must be taken against.
    ...
    CUDA_VISIBLE_DEVICES=1 python scripts/135_m3id_capability.py --model qwen3b --benchmark mmstar
"""
...
c95 = _load("_c95", "95_cot_baseline.py")        # tag -> (family, model_id, qwen_cls)

# m3id_baseline.json is keyed by the scripts/61 model key, not the scripts/95 tag.
