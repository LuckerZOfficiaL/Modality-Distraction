...
answer format differs, so any difference in the measured flip rate is attributable to the format.
    ...
    CUDA_VISIBLE_DEVICES=2 python scripts/freeform_pilot.py --model qwen2.5-vl-3b --n 420
...
REGISTRY = {
    ...
    # project's own loader, exactly as scripts/behavioral_eval.py does for the MCQ eval.
...
def _attach(model, spec):
    ...
    scripts/behavioral_eval.py does, so the free-form numbers sit on the same intervention."""
...
def main():
    ...
    # the paper's three reporting pools, same filters as scripts/behavioral_eval.py and make_table_pareto.py
    if args.pool == "test":
