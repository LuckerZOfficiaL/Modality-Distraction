...
--model / --sae-variant / --layer once the diversified SAEs are trained and X3 (scripts/44)
has been run to produce layer{L}_top.json.

    CUDA_VISIBLE_DEVICES=N python scripts/56_causal_ablation.py --model qwen \
...
def main() -> None:
    ...
    # so the sweep can take any top-K (self-contained; not limited to scripts/44's saved top).
