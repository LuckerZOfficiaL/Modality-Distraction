...
Trains on the D_M TRAIN split only (eval on test is leakage-free). Saves the adapter to --out.
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/finetune_distraction.py --method dropout --out runs/ft_dropout_qwen
"""
...
# Train-time messages are built byte-identical to the scripts/behavioral_eval.py make_hf eval format.
HF_SPECS = {
...
def make_tokenizer(family, proc, device, llmod=None, model_id=""):
    ...
    if family == "qwen":
        ...
        _c = importlib.util.spec_from_file_location(
            "_c6", ROOT / "collect_dm_counterfactual_activations.py")
    ...
    if family == "hf":  # generic HF VLM: mirror the scripts/behavioral_eval.py make_hf eval prompt exactly
        def _build(row, cap):
            content = []
            if cap is not None:
        ...
        # Must match the eval-side tokenization exactly (scripts/behavioral_eval.py make_hf): a train/eval BOS
