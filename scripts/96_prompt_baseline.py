...
Single forward pass, letter-logit argmax (same readout as scripts/61, NOT generation),
...
before trusting the three new backbones.
  ...
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python scripts/96_prompt_baseline.py --model qwen3b --pool both --out-suffix _promptbase_check
  # then the three missing models:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python scripts/96_prompt_baseline.py --model internvl --pool both
"""
