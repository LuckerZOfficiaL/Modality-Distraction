...
llava checkpoints are left in place since the SAE collection may be using them).
  ...
  default out: dm -> dm_multidomain_v1_cf ; assembled -> merged_aokvqa_racehigh_cf
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/collect_cf_anymodel.py --model qwen2.5-vl-3b --key qwen --pool assembled
    CUDA_VISIBLE_DEVICES=0 python scripts/collect_cf_anymodel.py --model OpenGVLab/InternVL3-8B-hf --pool both
"""
...
def setup(spec, device):
    ...
    if spec["type"] == "qwen":
        ...
        _c = importlib.util.spec_from_file_location("_c6", ROOT / "collect_dm_counterfactual_activations.py")
