...
Loads the model on CPU only to borrow `norm` + `lm_head` (compute is tiny matmuls).
    ...
    python scripts/logit_lens_distraction.py
"""
...
_s = importlib.util.spec_from_file_location("s65", _ROOT / "collect_cf_anymodel.py")
...
def main() -> None:
    ...
    ap.add_argument("--model", default="qwen", help="HF path or REGISTRY key (see scripts/behavioral_eval.py); cf-store dir = key")
    ...
    if del_id and not args.keep:
        ...
        try:
            s65.s61.delete_hf_cache(del_id)   # delete_hf_cache lives in scripts/behavioral_eval.py (s65 imports it as s61)
