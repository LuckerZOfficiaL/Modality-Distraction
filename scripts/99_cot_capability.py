...
scripts/78 scores capability by letter-logit argmax (one forward pass); CoT needs the model to
generate a reasoning chain and then be parsed for 'Answer: X'. This script pairs scripts/78's
benchmark loaders and registry with scripts/95's CoT instruction, generation settings and parser,
so the CoT row of the baseline comparison has the same capability column as every other row.

Parity with scripts/95 is deliberate and load-bearing: identical COT_INSTR, identical greedy
...
benchmarks are produced the same way as the CoT numbers on MoGround/assembled.
    ...
    python scripts/99_cot_capability.py --model qwen3b --benchmark mmstar --limit 400
"""
import argparse
...
c95 = _load("_c95", "95_cot_baseline.py")     # COT_INSTR, parse_letter, make_qwen/make_hf
c78 = _load("_c78", "78_benchmark_capability.py")   # REGISTRY + benchmark rows


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--model", required=True, help="scripts/95 registry tag, e.g. qwen3b, next")
