...
the fabricated caption stays scene-consistent and therefore adversarial rather than nonsensical.
    ...
    python scripts/adversarial_captions.py --generate
    # 2) merge into an augmented train split (CPU):
    python scripts/adversarial_captions.py --merge
    # 3) finetune Method A:
    CUDA_VISIBLE_DEVICES=0 python scripts/finetune_distraction.py --method hardneg \
