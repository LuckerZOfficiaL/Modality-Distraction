...
Reuses scripts/behavioral_eval.py's adapters + row loaders (download-run-delete). For one model, sweeps image
...
figure (all models/pools on the cross-model law) is built afterward by paper/make_figs.py.
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/grounding_knob_multimodel.py --model Qwen/Qwen2.5-VL-7B-Instruct
    CUDA_VISIBLE_DEVICES=0 python scripts/grounding_knob_multimodel.py --model qwen2.5-vl-3b --key qwen  # match law cell key
"""
...
_s = importlib.util.spec_from_file_location("s61", ROOT / "behavioral_eval.py")
...
def main() -> None:
    ...
    ap.add_argument("--model", required=True, help="HF path or REGISTRY key (see scripts/behavioral_eval.py)")
