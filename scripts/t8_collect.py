...
GPU (llavanext). Resumable (skips candidate_ids already written).
    ...
    python scripts/t8_collect.py --model qwen            # instant, CPU
    CUDA_VISIBLE_DEVICES=6 python scripts/t8_collect.py --model llavanext
    CUDA_VISIBLE_DEVICES=6 python scripts/t8_collect.py --model llavanext
"""
...
def main() -> None:
    ...
    if args.model == "qwen" and args.qwen_infer:
        ...
        _sp = importlib.util.spec_from_file_location("_c6", Path(__file__).parent / "collect_dm_counterfactual_activations.py")
